...
import {
	addTotalValue,
	findOccurrences,
	findOccurrencesProducts,
	getProductsRepMapFromArray,
	getSalesRepMapFromArray,
	highest,
	highestProduct,
} from '../utils/arrayM';
...
const CRMReportTemplate: FC<MyProps> = ({ tab }) => {
	...
	const checkPayment = async () => {
		...
		if (!paymentStatus) {
			toast.warn(
				'It appears your payment is due, please pay up to continue enjoying Digital Data Tree'
			);
	...
	const getClientsFromDB = () => {
		...
		if (tab == 5) {
				...
				.then((v) => {
					...
					if (v !== null) {
						...
						v.data.forEach((element) => {
							...
							if (
								decrypt(element.data().stage, id) === 'Project Started' ||
						...
						const uniqueProductsCal = salesByP.map(
							(value: any) => value.product
						);
							...
							datasets: [
								{
									...
									data: uniqueProductsCal.map((v: any) =>
										findOccurrencesProducts(prods, v)
									),
									backgroundColor: uniqueProductsCal.map(() =>
										randomRGBAColor()
									),
						...
						const uniqueSales = sls.reduce(
							(accumulator: any[], string: any) => {
			...
			getAllClientsByDate(first, last)
				.then((v) => {
					var infoFromCookie = '';
					if (getCookie(ADMIN_ID) == '') {
						infoFromCookie = getCookie(COOKIE_ID);
				...
				{loading ? (
					...
					<>
						{tempClients.length > 0 ? (
							<div className='shadow-2xl rounded-[25px] flex flex-col'>
								<div className='grid grid-cols-1 xs:flex xs:justify-between mb-6'>
									<h1 className='px-4 text-xl md:text-2xl lg:text-4xl'>
										Overview
									</h1>
									<button
										onClick={() => {
								...
								<div id='print'>
									<div className='grid grid-cols-1 smXS:grid-cols-2 md:grid-cols-3 lg:grid-cols-5'>
										<ReportHighlight
											title={'Top Product'}
											description={`${highestProduct(productsCount).value}`}
										/>
										<ReportHighlight
											title={'Top Sales Person'}
											description={`${highest(salesPeople).value}`}
										/>
										<ReportHighlight
											title={'Total Clients'}
										...
										/>
										<ReportHighlight
											title={'Convesion Rate'}
											description={`${Math.floor(
												(totalFinished.length / clients.length) * 100
											)}%`}
										/>
										<ReportHighlight
											title={'Total Value'}
									...
									<div className='w-full overscroll-contain overflow-x-auto whitespace-nowrap'>
										<table className='table border-separate border-spacing-1  shadow-2xl rounded-[25px] w-full'>
												...
												{tempClients.map((value, index) => {
													return (
														<tr
															key={index}
															className={
																'odd:bg-white even:bg-slate-50  hover:cursor-pointer'
															}
														>
															...
															<td className='text-left'>
																{value.enquired.map((v) => v.product + ', ')}
															</td>
											...
											<tfoot>
												<tr
													className={
														'bg-[#00947a] hover:cursor-pointer text-white'
													}
												>
													<td className='text-left'>
									...
									<div>
										{salesByProduct.map((v, i) => {
											...
											return (
												<div
													className='w-full overscroll-contain overflow-y-auto whitespace-nowrap'
													key={v.product}
												>
													<h1 className='ml-8'>{v.product} Total Quotations</h1>
														...
														<tbody>
															{v.value.map((value: any, index: any) => {
																if (typeof value !== 'undefined') {
																	if (
																		typeof value.enquired[i] !== 'undefined'
																	) {
																		totalValueOfSalesPerson += parseFloat(
																			value.enquired[i].value
																				.replace('$', '')
																				.replace(',', '')
																		);
																	}
																	...
																	return (
																		...
																		>
																			...
																			<td className='text-left'>
																				{typeof value.enquired[i] !==
																				'undefined'
																					? value.enquired[i].product
																					: 'N/A'}
																			</td>
																			<td className='text-left'>
																				{typeof value.enquired[i] !==
																	...
																	return <p>No Information to show as yet</p>;
																}
															})}
														</tbody>
														<tfoot>
															...
															>
																...
																<td className='text-left'>
																	$
																	{numberWithCommas(
																		totalValueOfSalesPerson.toFixed(2)
																	)}
																</td>
															</tr>
														</tfoot>
									...
									<div>
										{salesRepLabel.map((v) => {
											...
											return (
												<div
													className='w-full overscroll-contain overflow-x-auto whitespace-nowrap'
													key={v.person}
												>
													<h1 className='ml-8'>{v.person} Total Quotations</h1>
													<table className='table-auto border-separate border-spacing-1 p-4 w-full'>
														<thead className=' text-white font-bold w-full p-4'>
															<tr className='bg-[#00947a] py-3'>
																{labels.map((v: any) => (
															...
															{v.value.map((value: any, index: any) => {
																...
																return (
																	...
																	>
																		...
																		<td className='text-left'>{value.stage}</td>
																		<td className='text-left'>
																			{value.enquired.length > 0
																				? `${value.enquired[0].product}`
																				: 'Receipt'}
																		</td>
																		<td className='text-left'>{value.value}</td>
																	</tr>
							...
							<div>
								No Data yet, add a client or send a Quotation/Invoice to add
								information
							</div>
						)}
					</>
